#
#  SaplingCodonUsage.pm
#
package SaplingCodonUsage;
...
#===============================================================================
#  Sapling support for codon usages. Nothing is exported because the functions
#  are parallel to those for the SEED and KBase.
#===============================================================================
...
sub coding_sequences
{
    ...
    if ( $opts->{ region } )
    {
        ...
        $fid ||= '';
        $fid =~ s/^\d+\.\d+\..*$/fig|$fid/;
        my $gid2 = SeedUtils::genome_of( $fid );
        $fid && $gid2
        ...
        my $locH = $sap->fid_locations( { -ids => [$fid] } ) || {};
        my $loc  = $locH->{ $fid };
        $loc
        ...
        my $region = join( '_', $c, $l, $r );
        my $fidH = $sap->genes_in_region( { -locations => [$region] } ) || {};
        @fids    = grep { ftr_type($_) eq 'peg' } @{ $fidH->{ $region } || [] };
        $funcH   = $sap->ids_to_functions( { -ids => \@fids } ) || {};
    }
    else
    {
        $gid or return wantarray ? () : [];
        $funcH = $sap->feature_assignments( { -genome => $gid, -type => 'peg' } ) || {};
        @fids  = keys %$funcH;
    }
...
sub genome_codon_usages
{
    ...
    my $gid = $opts->{ gid } ||= shift  or return wantarray ? () : [];
    eval { require GenomeCodonUsages; } or return wantarray ? () : [];
    my $gf = $GenomeCodonUsages::genome_freqs{ $gid }
         or return wantarray ? () : [];

    my @usages;
    my $gname = $gf->{ gname } || '';
    my %key = ( average   => 'av_subtype',
                modal     => 'md_subtype',
                high_expr => 'he_subtype',
                nonnative => 'nn_subtype'
               );

    foreach ( qw( average modal high_expr nonnative ) )
    {
        my $fr = $gf->{ $_ } or next;
        my $st = $gf->{ $key{ $_ } } || '';
        push @usages, [ $gid, $gname, $_, $st, $fr, 1 ];
    }
...
sub genome_axes
{
    ...
    GenomeCodonUsages::genome_axes( $gid );
}
