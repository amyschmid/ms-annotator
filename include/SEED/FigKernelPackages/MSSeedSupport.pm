...
sub blast_sequence {
	...
	for (my $i=0; $i < @{$args->{sequences}};$i++) {
		...
		print TMP  $fastaString;
		close(TMP);
		system("blastall -i ".$filename." -d ".$self->figmodel()->config("blastdb cache directory")->[0].$dbname."/db.fasta -p blastn -FF -e 1.0e-5 -m 8 -o $filename.out 2> $filename.err");
...
sub getRastGenomeData {
	...
	my $output = {
		features => ModelSEED::FIGMODEL::FIGMODELTable->new(
            ["ID","GENOME","ESSENTIALITY","ALIASES","TYPE","LOCATION",
             "LENGTH","DIRECTION","MIN LOCATION","MAX LOCATION","ROLES",
             "SOURCE","SEQUENCE"],
            "NONE",["ID","ALIASES","TYPE","ROLES","GENOME"],"\t","|",undef),
		gc => 0.5,
		genome => $args->{genome},
		owner => "master"
	};
	...
	if ($output->{source} eq "MGRAST") {
		my $output = {
			source => "MGRAST",
			genome => $args->{genome},
			features => ModelSEED::FIGMODEL::FIGMODELTable::load_table(
                $figmodel->config("Metagenome directory")->[0].$args->{genome}.".tbl",
                "\t","|",0,["ID","GENOME","ROLES","SOURCE"]),
			name => "Unknown",
			taxonomy => "Metagenome",
			size => 0,
			owner => $figmodel->user()
    ...
    $output->{features} = $output->{features}->get_rows();
	return $output;
}
