...
sub genes_in_region
{
    ...
    my $relational_db_response = $rdbH->SQL( "SELECT id,location FROM features "
                                           ...
                                           . "   AND ( maxloc >= ? );",
                                             undef, $genome, $contig, $end, $minV, $beg
                                           ) || [];
    @$relational_db_response
        or return ( [], undef, undef );
...
sub all_subsystems_with_roles
{
    ...
    my $q = "SELECT DISTINCT i.subsystem, i.role  FROM subsystem_index i";
    my %seen;
...
sub get_all_subsystem_pegs {
    ...
    my $rdbH = $self->db_handle;
    # Declare the return variable.
    my $subList = $rdbH->SQL("SELECT subsystem, role, protein, variant " .
...

{
	...
	for my $ent (values %{$self->{cache}})
	{
	    next unless $ent->{genome} && $ent->{complete};
...
#	print "Destroy cache $self\n";
	...
	my $sth= $self->{fig}->db_handle->dbh->prepare("SELECT * FROM subsystem_metadata");
	$sth->execute;
