...
=head3 cache
Definition:
	REF:cached data in some form = FIGMODEL->cache(string:key);
Description:
	Returns data cached under the input key string.
=cut
sub cache {
	my($self,$key) = @_;
...
=head3 set_cache
Definition:
	string:error = FIGMODEL->set_cache(string:key,REF:data);
Description:
	Sets the cached value of key to the input data reference. If no errors, returns undefined.
=cut
sub set_cache {
	my($self,$key,$data) = @_;
...
=head3 debug_message
Definition:
	void FIGMODEL->debug_message(string);
Description:
...
sub filename {
	...
	my $hdl = $self->database()->get_object_manager("filename");
	$hdl->create({creationDate=>time(),finishedDate=>-1,folderExists=>0,user=>$self->user(),function=>$function});
	return $hdl->_id();
}
...
sub add_job_to_queue {
	my ($self,$args) = @_;
	$args = $self->process_arguments($args,["command"],{queue => "short",priority => 3,user => $self->user()});
	if (defined($args->{error})) {
	...
	$obj = $self->database()->create_object("job",{
		QUEUETIME => time(),
...
Definition:
	(string:Direction,string:Code,string:Reverse code,string:Full equation) = FIGMODEL->ConvertEquationToCode(string:Original equation,{string:compound id=>string:seed id});
Description:
...
sub ConvertEquationToCode {
	...
	if ($OriginalEquation =~ m/^\s[<=]/ || $OriginalEquation =~ m/^[<=]/ || $OriginalEquation =~ m/[=>]\s$/ || $OriginalEquation =~ m/[=>]$/) {
		$Error = 1;
		return ("",$OriginalEquation,"","",$EquationCompartment,$Error);
	}
	...
	return ($Direction,$Equation,$ReverseEquation,$FullEquation,$EquationCompartment,$Error);
}
...
Definition:
	(string)::search names = FIGMODEL->convert_to_search_name(string::name);
Description:
...
=head3 AddMetaGenomeModelToDB
Definition:
	(fail/success) = FIGMODEL->AddMetaGenomeModelToDB(string::metagenome id,string::user id,string::gapfilling);
Description:
=cut
sub AddMetaGenomeModelToDB {
	my ($self,$MetagenomeID,$Owner,$RunGapFilling) = @_;
	#Checking if the metagenome file exists
	if (!-e $self->config("raw MGRAST directory")->[0].$MetagenomeID.".summary") {
		$self->error_message("FIGMODEL:CreateMetaGenomeReactionList: could not find raw data file for metagenome ".$MetagenomeID);
		return $self->fail();
	}
	#Setting user ID if not provided
	if (!defined($Owner)) {
		$Owner = $self->user();
	}
	my $Users = $Owner;
	#Checking if the model already exsts
	my $model = $self->get_model("MGRast".$MetagenomeID);
	if (!defined($model)) {
		#If the model does not exist, we add it to the database
		$model = $self->database()->add_model_to_db("MGRast".$MetagenomeID,{status => [-2],owner => [$Owner],users => [$Users],rights => ["v"],genome => [$MetagenomeID],source => ["MGRAST"],date => [time()],overwrite => [1],message => ["Reconstruction queued"]});
	}
	#Creating directory for model owner
	if (!(-d $self->config("organism directory")->[0].$Owner."/")) {
		system("mkdir ".$self->config("organism directory")->[0].$Owner."/");
	}
	#Creating directory for model
	if (!(-d $model->directory())) {
		system("mkdir ".$model->directory());
	}
	if (defined($RunGapFilling) && $RunGapFilling eq "1") {
		$RunGapFilling = "?1";
	} else {
		$RunGapFilling = "";
	}
	#Scheduling reconstruction of the model
	$self->add_job_to_queue({command => "preliminaryreconstruction?".$model->id().$RunGapFilling,user => $Owner,queue => "short"});
	return $self->success();
}
...
sub createNewModel {
	...
	my $mdlobj = $self->database()->get_object("model",{id=>$id});
	if (defined($mdlobj)) {
		return {-error => $self->error_message("createNewModel:a model of ".$args->{-genome}." already exists.")};
	}
	#Setting the new model source
	my $genomeObj = $self->get_genome($args->{-genome});
	my $source = $genomeObj->source();
	my $name = $genomeObj->name();
	#Setting autocomplete media based on genome ID
	my $autocompleteMedia = "Complete";
	...
	$mdlobj = $self->database()->create_object("model",{ id => $id,
														 owner => $args->{-owner},
														 public => $public,
														 genome => $args->{-genome},
														 source => $source,
	...
	#Creating directory for model
	my $model = ModelSEED::FIGMODEL::FIGMODELmodel->new($self,$id,undef,$mdlobj);
	...
	my $RunGapFilling = "";
	if (!defined($args->{-gapfilling}) || $args->{-gapfilling} eq "1") {
		$RunGapFilling = "?1";
	}
	#Scheduling reconstruction of the model
	
	$self->add_job_to_queue({command => "preliminaryreconstruction?".$id.$RunGapFilling,queue => "short",user => $args->{-owner}});
	return undef;
}
...
Definition:
	([{DATABASE => [string],COMPARTMENT => [string],COEFFICIENT => [string]}],[{DATABASE => [string],COMPARTMENT => [string],COEFFICIENT => [string]}]) = FIGMODEL->GetReactionSubstrateData(string::reaction ID);
Description:
...
sub GetReactionSubstrateData {
	...
	my $Equation = $self->database()->GetDBTable("REACTIONS")->{$ReactionID}->[0]->{"EQUATION"}->[0];
	return $self->GetReactionSubstrateDataFromEquation($Equation);
}

=head3 GetReactionSubstrateDataFromEquation
Definition:
	([{DATABASE => [string],COMPARTMENT => [string],COEFFICIENT => [string]}],[{DATABASE => [string],COMPARTMENT => [string],COEFFICIENT => [string]}]) = FIGMODEL->GetReactionSubstrateDataFromEquation(string::equation);
Description:
	This function parses the input reaction equation and returns the data on reactants and products.
=cut
...
sub GenerateMFAToolkitCommandLineCall {
	...
	if (defined($ParameterFileList)) {
		...
		for (my $i=(@{$ParameterFileList}-1); $i >= 0; $i--) {
	...
	if (defined($MediaName) && length($MediaName) > 0 && $MediaName ne "NONE") {
		if ($MediaName eq "Complete" && $ModelName ne "Complete") {
			$CommandLine .= ' resetparameter "Default max drain flux" 100 resetparameter "user bounds filename" "NoBounds.txt"';
	...
	$self->debug_message("FIGMODEL->GenerateMFAToolkitCommandLineCall:".$CommandLine);
	return $CommandLine;
}
...
sub classify_database_reactions {
	...
	my $ReactionTB;
	my $UniqueFilename = $self->filename();
	system($self->GenerateMFAToolkitCommandLineCall($UniqueFilename,"Complete",$media,["ProductionCompleteClassification"],{"find tight bounds" => 1,"MFASolver" => "GLPK","Complete model biomass reaction" => $biomassrxn},"Classify-Complete-".$UniqueFilename.".log",undef,""));
...
Definition:
	$model->LoadProblemReport($Filename);
...
sub LoadProblemReport {
	...
	$Filename = $self->{"MFAToolkit output directory"}->[0].$Filename."/MFAOutput/ProblemReports.txt";
	if (-e $Filename) {
		return ModelSEED::FIGMODEL::FIGMODELTable::load_table($Filename,";","",0,undef);
	}
...
sub GeneLinks {
	...
	my $model = $self->get_model($SelectedModel);
	if (!defined($self->fig()) || !defined($model)) {
        return $GeneID;
	}

	my $fig = $self->fig($model->genome());
	if ($model->source() !~ m/^RAST/) {
		return '<a href="http://www.theseed.org/linkin.cgi?id=fig|'.$model->genome().".".$GeneID.'" title="'.$fig->function_of("fig|".$model->genome().".".$GeneID).'" target="_blank">'.$GeneID."</a>";
	}
	return '<a href="http://rast.nmpdr.org/seedviewer.cgi?page=Annotation&feature=fig|'.$model->genome().".".$GeneID.'" title="'.$fig->function_of("fig|".$model->genome().".".$GeneID).'" target="_blank">'.$GeneID."</a>";
}
...
sub process_models {
    ...
    for (my $i=$startNumber; $i < @{$objs}; $i++) {
    	...
    	if (!defined($owner) || $objs->[$i]->owner() eq $owner) {
	    	if ($objs->[$i]->id() =~ m/^Seed/ && $objs->[$i]->status() == -2) {
...
sub UpdateCompoundNamesInDB {
	...
	for (my $i=0; $i < @{$objs}; $i++) {
		...
		my $als = $self->database()->get_object("cpdals",{type => "name",COMPOUND => $objs->[$i]->id()});
		if (defined($als)) {
			$objs->[$i]->name($als->alias());
		}
	}
}
