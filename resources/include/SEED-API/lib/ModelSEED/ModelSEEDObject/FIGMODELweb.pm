...
sub get_model_overview_tbl {
    ...
    for (my $i=0; $i < @$modelList; $i++) {
	    ...
	    $row .= "<td> ".$model->name()."<br>(".$self->create_genome_link($model->genome()).")</td>";
	    if ($model->status() < 0) {
...
sub table_model_column {
	...
	if ($args->{type} eq "compound") {
		...
		if (!defined($cpd)) {
			return "Not in model";
		} elsif (defined($cpd->{COMPARTMENTS})) {
			my $output = "";
		...
		if ($model->source() =~ m/^MGRAST/ && defined($rxn->{REFERENCE}) && defined($rxn->{CONFIDENCE})) {
			...
			for (my $j=0; $j < @Roles; $j++) {
			  ...
			  $output .= '<a href="$FIG_Config::cgi_url/linkin.cgi?id=fig|'.$RoleHash->{$Roles[$j]}->{gene}.'" title="'.$Roles[$j].'" target="_blank">'.$RoleHash->{$Roles[$j]}->{fragments}." fragments:".$Score."</a>";
			}
			return $output;
		}
...
sub create_sbml_link {
	...
	my $Link = '<a  href="ModelSEEDdownload.cgi?model='.$model->id().'&file='.$type.'" target="_blank" title="'.$title.'">'.$name.'</a>';
	return $Link;
}
